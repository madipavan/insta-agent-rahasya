...
LEGAL_BLOCK = """
CRITICAL: Original Hindi paraphrase only. No verbatim book quotes. No recap phrases.
...
def build_script_prompt(
    ...
    cumulative_synopsis = _clip(cumulative_synopsis, _MAX_CONTEXT_CHARS)
    novel_logline = _clip(novel_logline, 200)
    story_summary = _clip(story_summary, _MAX_CONTEXT_CHARS)

    story_bible = ""
    if novel_logline or story_summary:
        story_bible = (
            f"\nNOVEL BIBLE (context only):\n"
    ...
    if episode_num > 1 and previous_episode:
        continuity_block = (
            f"\nCONTINUITY:\n"
            f"Last cliffhanger: {_clip(previous_episode.get('cliffhanger', ''), 160)}\n"
            f"Open in medias res from: {_clip(previous_episode.get('voiceover_excerpt', ''), 160)}\n"
        )
    elif episode_num == 1:
    ...
    return (
        ...
        f"Plot beat: {plot_beat}\n"
        f"Story so far: {cumulative_synopsis}\n"
